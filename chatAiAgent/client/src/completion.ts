...
export enum FinishReason {
    ...
    //ContentFilter = "content_filter", // TODO not implemented.
    //FunctionCall = "function_call", // deprecated, see "ToolCalls".
}
...
export interface _OaiApi_v1ChatCompletionRequest {
    ...
    parallel_tool_calls: boolean, // TODO untested => set to false.
}
...
// these are tool-calling-related.

export interface _OaiApi_v1ChatCompletionResponse_ToolCall {
    index: number|undefined; // for streaming mode only.
    id: string|undefined;
    type: ToolType|undefined;
    function: _OaiApi_v1ChatCompletionResponse_ToolCall_Function;
};

export interface _OaiApi_v1ChatCompletionResponse_ToolCall_Function {
...
export function deepCopy(arr: Array<_OaiApi_v1ChatCompletionResponse_ToolCall>): Array<_OaiApi_v1ChatCompletionResponse_ToolCall> {
    const result = new Array<_OaiApi_v1ChatCompletionResponse_ToolCall>();
    for ( const item of arr ) {
        const func: _OaiApi_v1ChatCompletionResponse_ToolCall_Function = {
            name: item.function.name,
            arguments: item.function.arguments,
        };
        const copy: _OaiApi_v1ChatCompletionResponse_ToolCall = {
            index: undefined,
            id: item.id,
            type: item.type,
            function: func,
